...
$(function() {
    ...
    $(document).on("click",'.subTabBigLink',function(){
        ...
        var hrefStr = $(this).parents(".subTabBig").find(".sub-menu .subTabLit").eq(0).find("a").attr("href").replace("javascript:f_addTab","")
            .replace("(","").replace(")","").replace(/\'/g,"");

        var hrefArr = hrefStr.split(",");
        f_addTab(hrefArr[0],hrefArr[1],hrefArr[2]);
    });
    ...
    $(document).on("click",'.subTabLit',function(){
     ...
     // f_addTab("welcome", "首页", "?r=admin/index/welcome");
...
function get_category(id,next,select_id){
    $.ajax({
        type : "GET",
        url  : '/index.php?r=admin/goods/parentcate&cate_id='+ id,
        dataType:'json',
